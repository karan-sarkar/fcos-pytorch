...
def compare(p, q):
    ...
    print(cls_p1.size(), box_p1.size(), center_p1.size())
    
    p1 = torch.cat([box_p1, center_p1], 1)
    p2 = torch.cat([box_p2, center_p2], 1)
    
    a1 = torch.einsum('na,nb->nbc', cls_p1, p1)
    a2 = torch.einsum('na,nb->nbc', cls_p2, p2)
